...
for (i in contrib) {
  ...
  g = data %>% 
    ...
    ggplot() + 
    geom_bar(aes(date, !!sym(i), fill = `abertura bcb`), stat = "identity") +
    geom_line(data = data %>% 
                group_by(date) %>% 
                reframe(date = date,
                        value = sum(!!sym(i))) %>% 
    ...
    labs(title = names(contrib)[m],
         subtitle = "IPCA-15 - Aberturas BCB",
         y = NULL, x = NULL,
  ...
  ggsave(paste0(names(contrib)[m], "_abertura_bcb.png"),
         width  = 4800,
         height = 2160,
         units  = "px",
         dpi    = 576,
...
for (i in unique(data$`abertura bcb`)) {
  ...
  ggsave(paste0(i,"_aberturas_bcb_qoq_annualized.png"),
         width  = 4800,
         height = 2160,
         units  = "px",
         dpi    = 576,
...
for (i in unique(data$`abertura bcb`)) {
  ...
  ggsave(paste0(i,"_aberturas_bcb_sazonal.png"),
         width  = 4800,
         height = 2160,
         units  = "px",
         dpi    = 576,
