...
for (i in var) {
  ...
  g = data %>% 
    select(date, servicos, !!sym(i)) %>% 
    drop_na() %>% 
    ggplot() +
    aes(x = date, y = !!sym(i), colour = servicos) + 
    geom_line(linewidth = .75, na.rm = FALSE) +
    facet_wrap(~servicos, scales = "free") +
    theme_bw() +
...
for (i in unique(data$servicos)) {
  ...
  ggsave(paste0(i,"_servicos_qoq_annualized.png"),
         width  = 4800,
         height = 2160,
         units  = "px",
         dpi    = 576,
...
for (i in unique(data$servicos)) {
  ...
  ggsave(paste0(i,"_servicos_bcb_sazonal.png"),
         width  = 4800,
         height = 2160,
         units  = "px",
         dpi    = 576,
