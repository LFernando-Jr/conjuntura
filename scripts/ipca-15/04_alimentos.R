...
for (i in var) {
  ...
  g = data %>% 
    select(date, alimentos, !!sym(i)) %>% 
    drop_na() %>% 
    ggplot() +
    aes(x = date, y = !!sym(i), colour = alimentos) + 
    geom_line(linewidth = .75, na.rm = FALSE) +
    facet_wrap(~alimentos, scales = "free") +
    theme_bw() +
    ...
    labs(title = "IPCA-15", 
         y = NULL, x = NULL, 
         subtitle = paste0(
           "Alimentos - ", case_when(!!sym(i) == "var_mom" ~ "m/m",
  ...
  ggsave(paste0(i, "_alimentos.png"),
         width  = 4800,
         height = 2160,
         units  = "px",
         dpi    = 576,
...
for (i in unique(data$alimentos)) {
  ...
  ggsave(paste0(i,"_alimentos_qoq_annualized.png"),
         width  = 4800,
         height = 2160,
         units  = "px",
         dpi    = 576,
...
for (i in unique(data$alimentos)) {
  ...
  ggsave(paste0(i,"_alimentos_bcb_sazonal.png"),
         width  = 4800,
         height = 2160,
         units  = "px",
         dpi    = 576,
