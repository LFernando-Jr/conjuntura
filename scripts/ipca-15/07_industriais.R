...
for (i in var) {
  ...
  g = data %>% 
    select(date, industriais, !!sym(i)) %>% 
    drop_na() %>% 
    ggplot() +
    aes(x = date, y = !!sym(i), colour = industriais) + 
    geom_line(linewidth = .75, na.rm = FALSE) +
    facet_wrap(~industriais, scales = "free") +
    theme_bw() +
    ...
    labs(title = "IPCA-15", 
         y = NULL, x = NULL, 
         subtitle = paste0(
           "Industriais - ", case_when(!!sym(i) == "var_mom" ~ "m/m",
  ...
  ggsave(paste0(i, "_industriais.png"),
         width  = 4800,
         height = 2160,
         units  = "px",
         dpi    = 576,
...
for (i in unique(data$industriais)) {
  ...
  ggsave(paste0(i,"_industriais_qoq_annualized.png"),
         width  = 4800,
         height = 2160,
         units  = "px",
         dpi    = 576,
...
for (i in unique(data$industriais)) {
  ...
  ggsave(paste0(i,"_industriais_bcb_sazonal.png"),
         width  = 4800,
         height = 2160,
         units  = "px",
         dpi    = 576,
